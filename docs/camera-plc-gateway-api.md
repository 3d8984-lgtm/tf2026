...
- **경고등이 두 대입니다** — `/api/v1/warning-light/*`(기존 장치, 5색+부저, 점멸 속도 조절
  가능)와 `/api/v2/d3v1-light/*`(신규 장치, 3색+부저, 점멸 속도 고정)는 **서로 다른 물리
  장치**입니다. 둘 다 deprecated 아님(프린터와 달리 기존 장치를 대체한 게 아니라 별도로
  추가됨) — 어느 쪽을 제어할지는 현장에 실제로 어떤 경고등이 있는지에 따라 다릅니다.
...
## USB 다층 경고등 API (v2, USB_D3V1)

새로 산 경고등용 API — 기존 `/api/v1/warning-light`(구형 장치)와는 완전히 별개
하드웨어입니다. 빨강/노랑/초록 3색 + 부저만 있고(파랑/흰색 없음), **점멸 "속도" 조절
기능이 없습니다** — `mode`가 `off`/`on`/`blink` 세 가지뿐이고 구형 장치의 `blink_level`
같은 필드는 없습니다.

### `GET /api/v2/d3v1-light/status`
...
### `POST /api/v2/d3v1-light/control`
...
`{ "mode": "off" }` \| `{ "mode": "on" }` \| `{ "mode": "blink" }` 중 하나 (구형 장치와
달리 `blink_level`은 없음 — 지정해도 무시됨).

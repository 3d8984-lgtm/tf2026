// Pure-JS MD5 (Deno WebCrypto does not support MD5, required by 4PX signing).
function toWords(s: Uint8Array): number[] {
  const n: number[] = [];
  for (let i = 0; i < s.length; i++) n[i >> 2] = (n[i >> 2] || 0) | (s[i] << ((i % 4) * 8));
  return n;
}
...
export function md5(input: string): string {
  ...
  const len = (((bitLen + 64) >>> 9) << 4) + 14;
  for (let i = 0; i <= len + 1; i++) x[i] = x[i] || 0;
  x[len] = bitLen;
